...
import ar.com.geneos.goizueta.plugin.client.pos.model.CreditCardPayment;
import ar.com.geneos.goizueta.plugin.client.pos.model.CreditNotePayment;
import ar.com.geneos.goizueta.plugin.client.pos.model.CreditPayment;
import ar.com.geneos.goizueta.plugin.client.pos.model.DiscountSchema;
import ar.com.geneos.goizueta.plugin.client.pos.model.EntidadFinancieraPlan;
...
public class PoSOnline extends PoSConnectionState {
	...
	public PoSOnline() {
		...
		setPaymentDocType(MDocType.getDocType(ctx, MDocType.DOCTYPE_CustomerReceipt, null));
	...
	private static void throwIfFalse(boolean b, DocAction sourceDocActionPO, Class posExceptionClass) throws PosException {
		if (!b) {
			...
			if (np != null) {
				...
				// Se intenta obtener un mensaje a partir del mensaje de los POs
				// que implementan DocAction.
			} else if (sourceDocActionPO != null && sourceDocActionPO.getProcessMsg() != null && sourceDocActionPO.getProcessMsg().length() > 0) {
	 ...
	 * 
	 * <ol>
	 * <li>Valida que el saldo sea cero (si no lo es, dispara una
	 * InsufficientBalanceException).
	 * 
	 * <li>Valida si tiene credito (si no lo tiene, dispara una
	 ...
	 * 
	 * @param order
	 *            la orden que se desea completar
	 * @throws PosException
	 * @throws InsufficientCreditException
	 */
	@Override
	public void completeOrder(Order order, Set<Integer> ordersId) throws PosException, InsufficientCreditException, InsufficientBalanceException,
			InvalidPaymentException, InvalidProductException {
		...
		try {
			...
			// DB.getSQLObject(getTrxName(),
			// "select 'Comenzando completeOrder'", null);
			...
			// * Aquí se completa la factura solo si debe ser emitida mediante
			// un controlador
			// * fiscal. Esta operación debe ser la última del conjunto dado que
			// una vez
			// * emitido el comrpobante impreso, la transacción debe confirmarse
			// para que
			// * los datos queden consistentes. (papel impreso con factura
			// creada en la BD).
			// * Si luego de esta operación se agrega otra (por ejemplo la
			// creación de otro
			// * documento) y esto falla, la transacción se anularía dejando
			// como resultado
			// * un comprobante impreso sin su factura correspondiente en la
			// base de datos.
			// */
			// if (invoice != null && needFiscalPrint(invoice)){
			// debug("Completando la factura para Impresión Fiscal");
			// throwIfFalse(invoice.processIt(DocAction.ACTION_Complete),
			// invoice, InvoiceCreateException.class);
			// }
			...
			/*
			 * try { if (morder != null && morder.getID() > 0) {
			 * morder.processIt(DocAction.ACTION_Void); morder.save(); } } catch
			 * (Exception e2) {}
			 * 
			 * try { if (invoice != null && invoice.getID() > 0) {
			 * morder.processIt(DocAction.ACTION_Void); morder.save(); } } catch
			 * (Exception e2) {}
			 * 
			 * try { if (shipment != null && shipment.getID() > 0) {
			 * shipment.processIt(DocAction.ACTION_Void); shipment.save(); } }
			 * catch (Exception e2) {}
			 */
			try {
				trx.rollback();
		...
		if (getShouldCreateInvoice() && invoice.requireFiscalPrint()) {
			...
			// que
			// cuando la emisión fiscal se hacía dentro de la transacción
			// principal.
	...
	public boolean reprintInvoice(Order order, FiscalDocumentPrint fdp) {
		...
		if (success) {
			try {
				// Impresión del documento de artículos a retirar por almacén
				printWarehouseDeliveryTicket(order);
				// Impresión del documento con datos del cliente en cuenta
				// corriente
				printCurrentAccountTicket(order);
			} catch (Exception e) {
	...
	/**
	 * Este método ajusta los pagos en efectivo, restándole lo que se le dió de
	 * vuelto al cliente.
	 * 
	 * Eso es, si el cliente tiene que pagar una orden por 85 y paga con 100, el
	 * pago en efectivo llega en 100 y acá se ajusta a 85, quitándole los 15 que
	 * se le devolvieron.
	 * 
	...
	private void clearState(Order order) {
		...
		// }
		//
		// trx = Trx.get(this.toString() + invoiceDate.toString() +
		...
		// Al completar el TicketTPV se crean la factura y el remito, con lo
		// cual en este caso
	...
	private void checkInvoice() throws PosException {
		...
		BigDecimal sumaPagos = sumaCashPayments.add(sumaCheckPayments).add(sumaCreditCardPayments).add(sumaCreditNotePayments).add(sumaBankTransferPayments);

		BigDecimal redondeo = VModelHelper.GetRedondeoMoneda(ctx, getClientCurrencyID());

		// Diff es la cantidad exacta que FALTA PAGAR para que la diferencia sea
		// PRECISAMENTE CERO.
		...
		if (sobraCheques.compareTo(totalPagar) > 0 && sumaCheckPayments.compareTo(BigDecimal.ZERO) > 0) {
	...
	private void checkSaldo(Order order) throws PosException {
		...
		// Si hay algun pago que no sea cheque, y están pagando de más, no
		// permito que se efectue la operacion

		BigDecimal redondeo = VModelHelper.GetRedondeoMoneda(ctx, getClientCurrencyID());
		boolean sobraPlata = sumaPagos.subtract(sumaProductos).compareTo(redondeo) >= 0;

		if (sobraPlata) { // order.getOrderProducts().size() !=
							// checkPayments.size()) {

			// if (sobraPlata)
			// throw new InvalidPaymentException();

			BigDecimal x = (sumaCreditPayments).add(sumaCreditCardPayments).add(sumaCreditNotePayments).add(sumaBankTransferPayments);
			...
			if (x.subtract(sumaProductos).compareTo(redondeo) > 0) {
				...
				// Falta plata -> de lo que tengo en efectivo, saco lo que me
				// falta pagar
		...
		for (int c = 0; c < cashPayments.size() && cashChange.compareTo(BigDecimal.ZERO) > 0; c++) {
			changeAux = cashPayments.get(c).getAmount().compareTo(cashChange) >= 0 ? cashChange : cashPayments.get(c).getAmount();
	...
	public ProductList searchProduct(String code) {
		...
		sql.append("SELECT DISTINCT ").append("u.M_Product_ID, ").append("bomPriceStd(u.M_Product_ID, M_PriceList_Version_ID, u.M_AttributeSetInstance_ID), ")
				.append("p.value, ").append("p.Name, ").append("bomPriceLimit(u.M_Product_ID, M_PriceList_Version_ID, u.M_AttributeSetInstance_ID), ")
				.append("u.M_AttributeSetInstance_ID, ").append("masi.description, ").append("u.MatchType, ").append("s.MandatoryType, ")
				.append("p.m_product_category_id, ").append("p.CheckoutPlace, ").append("p.IsSold ").append("FROM ( ");

		boolean needUnion = false; // Indicador de concatenación de UNION a la
									// consulta
		int codeParameterCount = 0; // Cantidad de veces que hay que agregar
									// 'code' como
									// parámetro de la consulta (varía según la
									// conf del TPV)
		String query = null;
		// Se agrega la sub-consulta de búsqueda por UPC en caso de que la
		// configuración
		...
		// Se agrega la sub-consulta de búsqueda por Clave de Búsqueda en caso
		// de que la
		...
		// Se agrega la sub-consulta de búsqueda por Nombre en caso de que la
		// configuración
		// del TPV así lo indique.
		...
		sql.append(") u ").append("INNER JOIN M_Product p ON (p.M_Product_id = u.M_Product_id) ")
				...
				.append("WHERE u.M_Product_ID = pp.M_Product_ID ").append("AND u.M_Product_ID = p.M_Product_ID ").append("AND p.IsActive = 'Y' ")
				.append("AND p.isSold = 'Y' ").append("ORDER BY u.MatchType ASC ");

		try {
			PreparedStatement pstmt = DB.prepareStatement(sql.toString());
			int i = 1;
			// Se carga el código del artículo como parámetro las veces que la
			// consulta
			// lo requiera
			...
			while (rs.next()) {
				...
				// mayor prioridad.
				if (productMatch.containsKey(m_Product_Id) && productMatch.get(m_Product_Id) <= matchType) {
					continue;
				}
				...
				product = new Product(m_Product_Id, code, productName, productPrice, productLimitPrice, M_AttributeSetInstance_ID, masiDescription,
						getPoSCOnfig().isPriceListWithTax(), getPoSCOnfig().isPriceListWithPerception(), masiMandatory, rs.getInt("m_product_category_id"),
						vendors, checkoutPlace, sold);
	...
	private void checkCredit(Order order) throws InsufficientCreditException, Exception {
		...
		result = manager.getTenderTypesToControlStatus(getCtx(), org, bp, getTrxName());
		// Si hubo error, obtengo el mensaje y tiro la excepción
		if (result.isError()) {
		...
		for (Payment pay : order.getPayments()) {
			...
			paymentRule = CurrentAccountBalanceStrategy.getPaymentRuleEquivalent(pay.getTenderType());
			if (!Util.isEmpty(paymentRule, true)) {
				if (tenderTypesAllowed != null && tenderTypesAllowed.contains(pay.getTenderType())) {
					shouldUpdateBPBalance = true;
					// Verificar la situación de crédito de la entidad comercial
					result = manager.validateCurrentAccountStatus(getCtx(), org, bp, creditStatus, getTrxName());
					// Si hubo error, obtengo el mensaje y tiro la excepción
					if (result.isError()) {
						throw new InsufficientCreditException(result.getMsg());
					}
				...
				convertedPayAmt = MConversionRate.convertBase(getCtx(), pay.getAmount(), pay.getCurrencyId(), order.getDate(), 0,
		...
		result = manager.checkInvoicePaymentRulesBalance(getCtx(), bp, org, pays, getTrxName());
		// Si hubo error, obtengo el mensaje y tiro la excepción
		if (result.isError()) {
			throw new InsufficientCreditException(result.getMsg());
		}
		...
		if (isCheckCUITControlActivated()) {
			...
			for (Payment payment : order.getPayments()) {
				if (payment.isCheckPayment()) {
					...
					auxCuit.replace("-", "");
					cuits.put(auxCuit, payment.getAmount().add(cuits.get(auxCuit) == null ? BigDecimal.ZERO : cuits.get(auxCuit)));
				}
			}
			// Validar que existan en la tabla de control de CUITS, si no están
			// se crean
			// Verificar que no superen el límite impuesto en el control y que
			// estén activos
			...
			for (String cuit : cuits.keySet()) {
				...
				if (!cuitControl.isActive()) {
					throw new Exception(Msg.getMsg(getCtx(), "CUITNotActive", new Object[] { cuit }));
				}
				...
				if ((balance.add(cuits.get(cuit))).compareTo(cuitControl.getCheckLimit()) > 0) {
					throw new Exception(Msg.getMsg(getCtx(), "CUITSurpassCheckLimit",
							new Object[] { cuit, cuitControl.getCheckLimit(), balance, cuits.get(cuit), balance.add(cuits.get(cuit)) }));
				}
			}
		}
	}
	 ...
	 */
	private Object performAditionalCurrentAccountWork(MOrg org, MBPartner bp, CurrentAccountManager manager, PO po, boolean addToWorkResults) throws Exception {
	...
	private MOrder createOxpOrder(Order order) throws PosException {
		...
		int paymentTermID = order.getPaymentTerm() != null ? order.getPaymentTerm().getId() : getPoSCOnfig().getPaymentTermID();
		...
		int numOrderLine = 0; // ADER: mejora de performance (y arreglo de bug),
								// al setearse explicitamente
		Map<Integer, Integer> manualLinesDiscounts = new HashMap<Integer, Integer>();

		for (OrderProduct op : products) {
			currentProduct++;

			// MProduct product = new MProduct(ctx, op.getProduct().getId(),
			// getTrxName());
			...
			// unicamente la ultima linea actualizará el encabezado con
		...
		// Descuentos: leer las líneas del pedido para ya tenerlas cacheadas y
		// setear
		...
		// borrador
		mo.setForceReserveStock(getPoSCOnfig().isCreateInOut() && getPoSCOnfig().isDraftedInOut());
		// Completar Orden
	...
	private void createOXPOrderTaxes(MOrder mo, Order order) throws PosException {
		BigDecimal totalNet = order.getOtherTaxes() != null && order.getOtherTaxes().size() > 0 ? mo.getTotalLinesNetWithoutDocumentDiscount()
				: BigDecimal.ZERO;
		MOrderTax orderTax;
	...
	protected MInvoice createOxpInvoice(Order order) throws PosException {
		...
		// Se indica que no se debe crear una línea de caja al completar la
		// factura ya
		// que es el propio TPV el que se encarga de crear los pagos e
		// imputarlos con
		// la factura (esto soluciona el problema de líneas de caja duplicadas
		// que
		// se había detectado).
		...
		for (MOrderLine line : moLines) {
			...
			// Se asigna el impuesto. Aquí se recalcula la tasa a partir de los
			// datos
			// de la factura.
			// IMPORTANTE: la nueva tasa puede variar de la que se calculó
			// previamente
			// en el ingreso de productos al pedido de TPV, dado que la entidad
			// comercial
			// puede diferir.
			invLine.setC_Tax_ID(line.getC_Tax_ID());

			// la ultima linea unicamente se encargá de setear los impuestos
			// correspondientes
		...
		//
		// Completar Factura.
		// IMPORTANTE: Si la factura debe ser emitida mediante un controlador
		// fiscal, no
		// se completa aquí dado que esto dispararía la impresión fiscal, y
		// luego, en caso
		// de producirse algún error por ejemplo al crear los pagos, quedaría
		// inconsistente
		// la información emitida con la guardada, dado que en caso de error no
		// se
		// guarda la factura OXP pero si podría emitirse el comprobante por la
		// impresora
		// fiscal. El completado y emisión de comprobante se deben hacer al
		// final del proceso
		// de creación del pedido.
		// if (!needFiscalPrint(inv)) {
		// throwIfFalse(inv.processIt(DocAction.ACTION_Complete), inv,
		// InvoiceCreateException.class);
		// }

		// Ignora la impresión fiscal al completar. Se hace luego fuera de la
		// transacción.
	...
	private void createOXPInvoiceTaxes(MInvoice mi, Order order) throws PosException {
		BigDecimal totalNet = order.getOtherTaxes() != null && order.getOtherTaxes().size() > 0 ? mi.getTotalLinesNet() : BigDecimal.ZERO;
		MInvoiceTax invoiceTax;
		for (Tax tax : order.getOtherTaxes()) {
			// Se hace un bypass de las percepciones ya que ya se deberían haber
			// creado
			if (!tax.isPercepcion()) {
	...
	private MInvoice createLocaleInvoice(Order order) throws PosException {
		...
		// Se obtiene la letra y el nro de punto de venta para determinar el
		// tipo
		...
		// config
		Integer posNumber = getPoSCOnfig().getPosNumberLetters().get(mLetraComprobante.getLetra());
		if (Util.isEmpty(posNumber, true)) {
			posNumber = getPoSCOnfig().getPosNumber();
		}

		// Se obtiene el tipo de documento para la factura.
		MDocType mDocType = MDocType.getDocType(ctx, Env.getAD_Org_ID(getCtx()), MDocType.DOCTYPE_CustomerInvoice, letra, posNumber, getTrxName());
		if (mDocType == null)
		...
		// Se asigna la letra de comprobante, punto de venta y número de
		// comprobante
		// a la factura creada.
		inv.setC_Letra_Comprobante_ID(mLetraComprobante.getID());
		inv.setPuntoDeVenta(posNumber);
		// Nro de comprobante.
		// String documentNo = MSequence.getDocumentNo(mDocType.getID(),
		// getTrxName());
		...
		// MCategoriaIva mCategoriaIvaCus = new MCategoriaIva(ctx,
		// partner.getC_Categoria_Iva_ID(), getTrxName());
		String cuit = partner.getTaxID();
		inv.setCUIT(cuit);

		// Siempre se asignan los datos del comprador
		// Se asignan los datos de consumidor final.
		// Nombre, dirección e identificación para los casos en que el monto de
		// la factura
	...
	private MInOut createOxpInOut(Order order) throws PosException {
		 ...
		 * 
		 * El remito ahora no se crea con la totalidad de los artículos que se
		 * agregaron al pedido del TPV sino que solo se agregan aquellos
		 * artículos cuyo lugar de retiro sea el TPV. Los artículos cuyo retiro
		 * sea por almacén no serán incluidos en este remito, quedando sus
		 * cantidades pendientes en el pedido previamente creado.
		 */
		shipment = null;
		// El remito se realiza solo si la configuración del TPV así lo indica y
		// además
		// si el pedido de TPV contiene al menos 1 artículo cuyo retiro sea por
		// TPV (si
		// todos los artículos del pedido se retiran por almacén entonces el
		// remito no
		...
		if (getPoSCOnfig().isCreateInOut() && order.getPOSCheckoutProductsCount() > 0) {
			...
			// Ader: evitar que se reelean las MOrderLInes; esto evita 20
			// accesos
			// y otros tantos potencialemten (al evitar acceer a la cache
			// tradcionale
			// de productos
			HashMap<Integer, MOrderLine> mapMo = new HashMap<Integer, MOrderLine>();
			// no deberia ser necesario getLines(true); si deberia, que se
			// resete
			...
			// Se recorren todos los artículos del pedido TPV para ir
			// agregándolos
			// al remito. Es necesario iterar sobre esta colección (y no sobre
			// los
			// MOrderLines del mOrder creado previamente) ya que en la colección
			// se encuentra el lugar de retiro del artículo, y de este dato
			// depende
			// si el artículo se agrega o no al remito (a diferencia del MOrder
			// y MInvoice
			// que se crean con todas las líneas cargadas al TPV).
			for (OrderProduct orderProduct : order.getOrderProducts()) {
				// El remito solo incluye los artículos que se retiren por el
				// TPV
				// Aquellos artículos cuyo retiro sea por almacén quedan fuera
				// de este
				// remito, quedando pendientes de entrega.
				if (orderProduct.isPOSCheckout()) {
					// Obtiene la instancia de la línea de pedido creada.
					/*
					 * MOrderLine orderLine = new MOrderLine( morder.getCtx(),
					 * orderProduct.getOrderLineID(), getTrxName());
					 */
	...
	private MPayment createOxpMPayment(String TenderType, int C_Currency_ID, BigDecimal Amt, String documentNo) {
		...
		// Aquí se está asignando los datos de la cuenta bancario destino del
		// cobro, mientras
		// que esos campos existen para ingresar la información relacionada con
		// el ORIGEN
		// del cobro. Con lo cual se está mezclando la información y esto
		// confunde al usuario
		// En la ventana de Cobros estos campos son visibles solo para Cheques y
		// Transferencias
		// con lo cual, se deberían cargar con la identificación del Banco y la
		// Cuenta origen
		// del Cheque o Transferencia.
		// Por el momento se comentan estas línas para no seguir cargando
		// erróneamente
		// esta información.
		// -->
	...
	private MAllocationLine createOxpMAllocationLine(Payment p, MPayment pay, MCashLine cashLine, Integer creditInvoiceID) throws PosException {
		return createOxpMAllocationLine(getShouldCreateInvoice() ? invoice.getC_Invoice_ID() : 0, p, pay, cashLine, creditInvoiceID);
	}
	 ...
	 * @throws PosException
	 */
	private MAllocationLine createOxpMAllocationLine(Integer debitInvoiceID, Payment p, MPayment pay, MCashLine cashLine, Integer creditInvoiceID)
			throws PosException {
		return createOxpMAllocationLine(debitInvoiceID, p, pay, cashLine, creditInvoiceID, null);
	}

	private MAllocationLine createOxpMAllocationLine(Integer debitInvoiceID, Payment p, MPayment pay, MCashLine cashLine, Integer creditInvoiceID,
			BigDecimal amount) throws PosException {
		BigDecimal allocLineAmt = currencyConvert(amount == null ? p.getAmount().abs() : amount.abs(), p.getCurrencyId(), allocHdr.getC_Currency_ID());
	...
	private void createOxpPayments(Order order) throws PosException {
		...
		// Completar Pagos. En este punto ya están creadas y guardadas las
		// lineas de imputacion (MAllocationLine),
		// por lo que puedo completarlos sin que se generen conflictos con la
		// imputacion automática.
	...
	private void createOxpCashPayment(CashPayment p) throws PosException {
		createOxpCashPayment(getShouldCreateInvoice() ? invoice.getC_Invoice_ID() : 0, p, true, true);
	}

	private MCashLine createOxpCashPayment(Integer debitInvoiceID, CashPayment p, Integer chargeID, boolean addToCashLineList, boolean addToAllocation)
			throws PosException {
		// MCashBook cashBook = new MCashBook(ctx, posConfig.getC_CashBook_ID(),
		// trxName);
		MCash cash = null;
		// Si el config tiene asociado el Cash entonces se usa ese (Cajas
		// Diarias)
		...
		throwIfFalse(cashLine.save(), cashLine); // Necesario para que se asigne
													// el C_CashLine_ID
	...
	private void createOxpCheckPayment(CheckPayment p) throws PosException {
		...
		String sucursal = VModelHelper.getSQLValueString(null, " select AccountNo from c_bankaccount where c_bankaccount.c_bankaccount_id = ? ",
				p.getBankAccountID());
		...
		pay.setMicr(sucursal + ";" + p.getBankAccountID() + ";" + p.getCheckNumber()); // Sucursal;
																						// cta;
																						// No.
																						// cheque
	...
	private void createOxpCreditCardPayment(CreditCardPayment p, Order order) throws PosException {
		...
		amount = amount.add(Util.isEmpty(p.getChangeAmt(), true) ? BigDecimal.ZERO : p.getChangeAmt());
		...
		pay.setA_Name(!Util.isEmpty(order.getBusinessPartner().getCustomerName(), true) ? order.getBusinessPartner().getCustomerName() : order
	...
	private void createCreditCardRetirementInvoice(CreditCardPayment p, MPayment pay, Order order) throws PosException {
		...
		// config del tpv
		creditCardRetirementInvoice.setC_DocTypeTarget_ID(getPoSCOnfig().getCreditCardCashRetirementDocTypeID());
		creditCardRetirementInvoice.setC_DocType_ID(getPoSCOnfig().getCreditCardCashRetirementDocTypeID());
		...
		// Guardo la factura
		throwIfFalse(creditCardRetirementInvoice.save(), creditCardRetirementInvoice, InvoiceCreateException.class);
		...
		// Artículo de retiro de efectivo de tarjeta de crédito de la config del
		// tpv
		creditCardRetirementInvoiceLine.setM_Product_ID(getPoSCOnfig().getCreditCardCashRetirementProductID());

		// Impuesto Exento
		creditCardRetirementInvoiceLine.setC_Tax_ID(getTaxExento() != null ? getTaxExento().getID() : 0);
		creditCardRetirementInvoiceLine.setSkipManualGeneralDiscount(true);

		// Guardo la línea
		throwIfFalse(creditCardRetirementInvoiceLine.save(), InvoiceCreateException.class);
		// Completo la factura
		throwIfFalse(creditCardRetirementInvoice.processIt(DocAction.ACTION_Complete), creditCardRetirementInvoice, InvoiceCreateException.class);
		// Guardo la factura
		throwIfFalse(creditCardRetirementInvoice.save(), creditCardRetirementInvoice, InvoiceCreateException.class);
		// Creo la línea del allocation
		createOxpMAllocationLine(creditCardRetirementInvoice.getID(), p, pay, null, null, p.getChangeAmt());
		...
		cashPayment.setDescription(Msg.parseTranslation(getCtx(), "@CashRetirement@" + " " + "@CouponNumber@" + " " + p.getCouponNumber()));
		// Cargo de retiro de efectivo de tarjeta de crédito de la config del
		// tpv
		MCashLine cashLine = createOxpCashPayment(0, cashPayment, getPoSCOnfig().getCreditCardCashRetirementChargeID(), false, false);
	...
	private void createOxpCreditNotePayment(CreditNotePayment p) throws PosException {
		// No se debe crear ningún documento de pago ya que la nota de crédito
		// ya
		// existe en el sistema. Solo se hace la imputación contra la factura
		// del pedido
		...
		if (p.isReturnCash() && p.getChangeAmt() != null && p.getChangeAmt().compareTo(BigDecimal.ZERO) > 0) {
			CashPayment cashPayment = new CashPayment(p.getChangeAmt().negate());
	...
	private MAllocationHdr createOxpAllocation() throws PosException {
		...
		hdr.setGrandTotal(approvalAmt); // GrandTotal = approvalAmt -
										// Retenciones
	...
	protected MCheckCuitControl createCheckCUITControl(Integer orgID, String cuit) throws Exception {
		...
		MCheckCuitControl cuitControl = (MCheckCuitControl) PO.findFirst(getCtx(), X_C_CheckCuitControl.Table_Name,
				"ad_org_id = ? AND translate(upper(trim(cuit)), '-', '') = translate(upper(trim('" + cuit + "')), '-', '')", new Object[] { orgID }, null,
				getTrxName());
		// Si no existe lo creo
		if (cuitControl == null) {
			cuitControl = new MCheckCuitControl(getCtx(), 0, getTrxName());
			cuitControl.setCUIT(cuit);
			cuitControl.setCheckLimit(MCheckCuitControl.getInitialCheckLimit(Env.getAD_Org_ID(getCtx()), getTrxName()));
	...
	private void doCompleteAllocation() throws PosException {
		...
		if (allocLines.size() > 0) {
			...
			/*
			 * Si el medio de pago es crédito no se crea ningún elemento de pago
			 * ya que la factura creada quedará con un saldo a pagar.
			 * 
	...
	public BusinessPartner getBPartner(int bPartnerID) {
		...
		// Se toma siempre el M_DiscountSchema_ID de la EC ya que es transacción
		// de ventas.
		...
		BusinessPartner rBPartner = new BusinessPartner(bPartnerID, 0, mBPartner.getTaxID(), mBPartner.getName(), mBPartner.getM_PriceList_ID(),
				discountSchema, mBPartner.getFlatDiscount(), paymentTerm, paymentMedium);
		...
		// Si no es la misma EC que la por defecto en la config, se cargan los
		// datos
		...
		if (bPartnerID != getPoSCOnfig().getBPartnerCashTrxID()) {
			...
			// Indica que los datos del comprador se deben mantener
			// sincronizados
	...
	public boolean productStockValidate(int productId, BigDecimal count, int attributeSetInstanceID) {
		...
		if (mProduct.isStocked()) {
			...
			// Se consulta el stock para el articulo con instancia de atributo
			// si la misma
			// existe.
			...
			if (availableCount == null) {
				...
				stockAvailable = (availableCount.compareTo(count) >= 0);
	...
	public void loadEntidadesFinancieras() {
		...
		String sql = "SELECT M_EntidadFinanciera_ID, Name, CardMask, isallowcreditcardcashretirement, creditcardcashretirementlimit "
				+ "FROM M_EntidadFinanciera " + "WHERE ((C_City_ID IS NULL) OR (C_City_ID = ?)) AND (AD_Org_ID = ? OR AD_Org_ID = 0) AND IsActive = 'Y' ";

		// Se comenta esta línea ya que se cargas las entidades financieras de
		// la organización actual o de la 0
		// sql = MRole.getDefault().addAccessSQL( sql, "M_EntidadFinanciera",
		// false, true );
		...
		try {
			...
			while (rs.next()) {
				...
				EntidadFinanciera entidad = new EntidadFinanciera(c_EntidadFinanciera_ID, name, rs.getString("CardMask"), rs.getString(
						"isallowcreditcardcashretirement").equals("Y"), rs.getBigDecimal("creditcardcashretirementlimit"));
	...
	public List<PoSConfig> getPoSConfigs() {
		...
		if (MPOSJournal.isActivated()) {
			...
			String sqlWhere = " AD_Client_ID = " + AD_Client_ID + " AND AD_Org_ID = " + AD_Org_ID + " AND SalesRep_ID = " + AD_User_ID + " AND IsActive = 'Y' "
	...
	public void printTicket() throws Exception {
		...
		int tableID = 0;
		int recordID = 0;
		// Por defecto se lanza el informe configurado en la pestaña de
		// encabezado
		// del pedido.
		...
		// En caso de que se haya emitido una factura, entonces se lanza el
		// informe
		// de la factura.
		if (getShouldCreateInvoice()) {
			...
			if (docType.getAD_Process_ID() > 0) {
				...
				// Si el tipo de documento no tiene un informe asociado, se
				// utiliza el
				// informe configurado para la ventana de Facturas de Clientes.
			} else {
				processID = new M_Tab(ctx, INVOICE_TAB_ID, null).getAD_Process_ID();
			}
		...
		if (processID > 0) {
			...
			ProcessCtl.process(getProcessListener(), 0, pi, null); // calls
																	// lockUI,
																	// unlockUI
		} else {
	...
	private MPrintFormat getTicketPrintFormat() {
		...
		// 2. Si no encuentra un formato específico para la organización,
		// obtiene
		...
		String sql = " SELECT AD_PrintFormat_ID " + " FROM ( " + "	(SELECT Order_Printformat_ID AS AD_PrintFormat_ID " + "	FROM AD_PrintForm "
				+ "	WHERE AD_Client_ID = ? AND AD_Org_ID = ?) " +

				"	UNION " +

				" 	(SELECT Order_Printformat_ID AS AD_PrintFormat_ID " + "	FROM AD_PrintForm " + "	WHERE AD_Client_ID = ?) " +

				"	UNION " +

				"	(SELECT AD_PrintFormat_ID " + "	FROM C_DocType " + "	WHERE C_DocType_ID = ?) " + " ) f " + " WHERE AD_PrintFormat_ID IS NOT NULL ";
	...
	public Product getProduct(int productId, int attributeSetInstanceId) {
		...
		/*
		 * sql =
		 * "SELECT p.M_Product_ID, pp.PriceStd, p.Name, pp.PriceLimit, p.upc " +
		 * "FROM M_ProductPrice pp, M_Product p " +
		 * "WHERE p.M_Product_ID = pp.M_Product_ID " +
		 * "	 AND p.M_Product_ID = ? " + "  AND M_PriceList_Version_ID = ? " +
		...
		try {
			...
			if (rs.next()) {
				...
				boolean masiMandatory = MAttributeSet.MANDATORYTYPE_AlwaysMandatory.equals(rs.getString("MandatoryType"));
				String masiDescription = (String) DB.getSQLObject(null, "SELECT Description FROM m_attributesetinstance WHERE m_attributesetinstance_id = ?",
						new Object[] { attributeSetInstanceId });
				vendorsIDs = getVendors(m_Product_Id);
				String checkoutPlace = rs.getString("CheckoutPlace");
				boolean sold = "Y".equals(rs.getString("IsSold"));
				// Creo el producto.
				product = new Product(m_Product_Id, code, productName, productPrice, productLimitPrice, attributeSetInstanceId, masiDescription, getPoSCOnfig()
						.isPriceListWithTax(), getPoSCOnfig().isPriceListWithPerception(), masiMandatory, rs.getInt("m_product_category_id"), vendorsIDs,
						checkoutPlace, sold);
			}
	...
	public Tax getProductTax(int productId, int locationID) {
		...
		int taxId = org.openXpertya.model.Tax.get(ctx, productId, 0, now, now, Env.getAD_Org_ID(ctx), getPoSCOnfig().getWarehouseID(), locationID, locationID,
				true);
	...
	/**
	 * Crea un <code>OrderProduct</code> a partir de una línea de un pedido del
	 * sistema (C_OrderLine)
	 * 
	 * @param line
	 *            Línea del pedido.
	...
	private OrderProduct createOrderProduct(MOrderLine line) {
		...
		// Si el TPV no crea remito siempre el retiro es por almacén. Además, si
		// crea remito
		// y está configurado para que los pedidos pre-creados agregados al
		// pedido TPV sean
		// entregados por Almacén, entonces el retiro también es por almacén.
		if (!getPoSCOnfig().isCreateInOut() || getPoSCOnfig().isDeliverOrderInWarehouse()) {
			checkoutPlace = MProduct.CHECKOUTPLACE_Warehouse;
			// Aquí, se crean remitos y los pedidos pre-creados agregados se
			// incluyen en
		...
		orderProduct = new OrderProduct(line.getQtyEntered(), line.getDiscount(), new Tax(mTax.getID(), mTax.getRate(), mTax.isPercepcion()), product,
				checkoutPlace);
	...
	public List<PriceList> getPriceLists() {
		...
		List<PO> priceLists = PO.find(Env.getCtx(), "m_pricelist", "ad_client_id = ? and isactive = 'Y' and issopricelist = 'Y'",
				new Object[] { Env.getAD_Client_ID(Env.getCtx()) }, null, null);
		...
		for (PO priceList : priceLists) {
			...
			newPriceList = new PriceList(mPriceList.getID(), mPriceList.getName(), mPriceList.getDescription(), mPriceList.getC_Currency_ID(),
					mPriceList.isTaxIncluded(), mPriceList.isPerceptionsIncluded(), mPriceList.isSOPriceList(), mPriceList.isDefault(),
					mPriceList.getPricePrecision());
	...
	public PriceList getCurrentPriceList(int windowNo) {
		...
		PriceList newPriceList = new PriceList(priceList.getID(), priceList.getName(), priceList.getDescription(), priceList.getC_Currency_ID(),
				priceList.isTaxIncluded(), priceList.isPerceptionsIncluded(), priceList.isSOPriceList(), priceList.isDefault(), priceList.getPricePrecision());
		return newPriceList;
	}
	...
	public PriceListVersion getCurrentPriceListVersion(PriceList priceList, int windowNo) {
		...
		String SQL = "SELECT plv.M_PriceList_Version_ID, plv.ValidFrom " + "FROM M_PriceList pl, M_PriceList_Version plv "
				+ "WHERE pl.M_PriceList_ID=plv.M_PriceList_ID" + " AND plv.IsActive='Y'" + " AND pl.M_PriceList_ID=? " // 1
		...
		try {
			...
			if (versionID != 0) {
				...
				version = new PriceListVersion(pLVersion.getID(), pLVersion.getName(), pLVersion.getDescription(), pLVersion.getM_DiscountSchema_ID(),
		...
		// Env.setContext(
		// Env.getCtx(),windowNo,"M_PriceList_Version_ID",retValue );
		return version;
	}
	...
	public BigDecimal getProductPrice(Product product, PriceListVersion priceListVersion) {
		...
		if (priceListVersion != null && product != null) {
			PO productPrice = PO.findFirst(Env.getCtx(), "m_productprice", "m_product_id = ? AND m_pricelist_version_id = ?", new Object[] { product.getId(),
					priceListVersion.getId() }, new String[] { "created desc" }, null);
	...
	/**
	 * @return Devuelve la sub-consulta para búsqueda exacta y/o parcial de
	 *         artículos por UPC
	 */
	private String getSearchByUPCQuery() {
		StringBuffer query = new StringBuffer();
		// La consulta base contiene partes sin completar encerradas entre <>.
		// Estos tags
		// son luego reemplazados por valores concretos según la necesidad de
		// hacer una
		// búsqueda exacta y/o parcial. De esta forma la consulta se esribe una
		// única vez
		// y se reutiliza en ambas búsquedas.
		StringBuffer baseQuery = new StringBuffer();
		baseQuery
				// 1. Busca en los UPC de instancias. Estos son los que tiene
				// mayor prioridad
				...
				.append("UNION ")
				// 2. Busca en los UPC asociados al artículo. Primero se lista
				// el UPC predeterminado.
				.append("(SELECT M_Product_ID, 0, <MT_M_ProductUPC> ").append("FROM M_ProductUPC ")
				.append("WHERE (UPPER(UPC) <COMPARATOR> UPPER(<VALUE>)) AND IsActive = 'Y' ").append("ORDER BY IsDefault DESC) ")
				.append("UNION ")
				// 3. Buscan por UPC en los artículos asociados al proveedor
				.append("(SELECT M_Product_ID, 0, <MT_M_Product_PO> ").append("FROM M_Product_PO ")
				.append("WHERE (UPPER(UPC) <COMPARATOR> UPPER(<VALUE>)) AND IsActive = 'Y') ").append("UNION ")
				// 4. Busca por VendorProductNo en los artículos asociados a
				// entidades comerciales
				.append("(SELECT M_Product_ID, 0, <MT_C_BPartner_Product> ").append("FROM C_BPartner_Product ")
				.append("WHERE (UPPER(VendorProductNo) <COMPARATOR> UPPER(<VALUE>)) AND isActive = 'Y') ");

		String exactQuery = null;
		String partialQuery = null;

		// La configuración de TPV indica que hay que realizar una búsqueda
		// exacta por UPC
		if (getPoSCOnfig().isSearchByUPC()) {
			exactQuery = baseQuery.toString();
			// Reemplazada el comparador y el valor. Aquí se compara por
			// igualdad exacta
		...
		if (getPoSCOnfig().isSearchByUPCLike()) {
			...
			// Reemplazada el comparador y el valor. Aquí se compara por ILIKE
			// para no tener
			// en cuenta mayúsculas y minúsculas, y solo se agrega un comodín al
			// final de la
			// cadena. Debido a que los UPC son comúnmente numéricos, si se
			// agregara un
			// comodín al inicio también produciría demasiados resultados
			// desechables.
			...
			// Si se requirió la búsqueda exacta es necesario unir los
			// resultados con esta
			// búsqueda.
			if (exactQuery != null) {
				query.append("UNION ");
	...
	/**
	 * @return Devuelve la sub-consulta para búsqueda exacta y/o parcial de
	 *         artículos por Clave de Búsqueda
	 */
	private String getSearchByValueQuery() {
		...
		baseQuery
				...
				.append("(SELECT M_Product_ID, 0 AS M_AttributeSetInstance_ID, <MT_M_Product> AS MatchType ").append("FROM M_Product ")
	...
	private String getSearchByNameQuery() {
		...
		if (getPoSCOnfig().isSearchByName()) {
			...
			// Reemplazada el comparador y el valor. Aquí se compara por
			// igualdad de strings
			// sin case sentsitive para generar mejores resultados debido a que
			// los nombres
			// de artículos no se diferencian por mayúsculas o minúsculas sino
			// por la propia
			// cadena de nombres.
		...
		if (getPoSCOnfig().isSearchByNameLike()) {
			...
			// Reemplazada el comparador y el valor. Aquí se compara por ILIKE
			// para no tener
			// en cuenta mayúsculas y minúsculas, y se agregan un comodines al
			// inicio y
			// final de la cadena.
			partialQuery = partialQuery.replaceAll("<COMPARATOR>", "ILIKE");
			partialQuery = partialQuery.replaceAll("<VALUE>", "('%' || ? || '%')");
	...
	/**
	 * Devuelve la cantidad de parámetros que requiere una consulta SQL para se
	 * utilizada como una {@link PreparedStatement}. Simplemente cuenta la
	 * cantidad de caracteres <code>?</code> que existen dentro del String.
	 * 
	 * @param sql
	 *            Consulta SQL origen
	...
	/**
	 * Imprime el comprobante para retiro de artículos por almacén en caso de
	 * estar indicada esta opción en la configuración del TPV.
	 */
	public void printWarehouseDeliveryTicket(Order order) throws Exception {
		/*
		 * Por el momento SOLO se imprime el comprobante mediante una Impresora
		 * Fiscal, con lo cual se deben cumplir estas condiciones: 1) Está
		 * activada la localización Argentina 2) El TPV está configurado para
		 * realizar la factura 3) El tipo de documento de la factura está
		 * configurado para que se emita el comprobante mediante un controlador
		 * fiscal, teniendo este tipo asociada la impresora fiscal a utilizar
		 * 
		 * Al cumplirse estas 3 condiciones, aquí se obtiene la impresora fiscal
		 * asociada al tipo de documento de la factura creada, y se emite el
		 * ticket para retiro x almacén en esa impresora fiscal.
		 */
		...
		// documento de retiro
		if (getPoSCOnfig().isPrintWarehouseDeliverDocument() && invoice != null && order.getWarehouseCheckoutProductsCount() > 0) {
			// El tipo de documento de la factura debe ser fiscal y tener
			// asociado
			...
			// se deben retirar por almacén
				...
				ProcessInfo info = MProcess.execute(getCtx(), MInvoice.getWarehouseDeliverDocumentProcessID(null), params, null);
				if (info.isError()) {
	...
	public void printCurrentAccountTicket(Order order) throws Exception {
		...
		if (getPoSCOnfig().isPrintCurrentAccountDocument()
				&& invoice != null
				&& (!Util.isEmpty(currentAccountSalesConditions.keySet()) || (order.getBusinessPartner().isAutomaticCreditNote() && sumaCreditNotePayments
						.compareTo(BigDecimal.ZERO) > 0))) {
			MBPartner partner = new MBPartner(getCtx(), order.getBusinessPartner().getId(), null);
			// Itero por las condiciones de venta de cuenta corriente
			for (String paymentRule : currentAccountSalesConditions.keySet()) {
				currentAccountInfo = new CurrentAccountInfo(null, MRefList.getListName(getCtx(), MInvoice.PAYMENTRULE_AD_Reference_ID, paymentRule),
			...
			if (order.getBusinessPartner().isAutomaticCreditNote()) {
				if (infoOnCredit != null) {
					infoOnCredit.setAmount(infoOnCredit.getAmount().add(sumaCreditNotePayments));
				} else {
					currentAccountInfo = new CurrentAccountInfo(null, MRefList.getListName(getCtx(), MPOSPaymentMedium.TENDERTYPE_AD_Reference_ID,
							MPOSPaymentMedium.TENDERTYPE_CreditNote), sumaCreditNotePayments);
					infos.add(currentAccountInfo);
				}
			}
			...
			if (docType.getC_Controlador_Fiscal_ID() > 0) {
				...
				fdp.setPrinterEventListener(getFiscalPrinterEventListener());
				MInvoice invoiceAux = new MInvoice(getCtx(), invoice.getID(), null);
				if (!fdp.printCurrentAccountDocument(docType.getC_Controlador_Fiscal_ID(), partner, invoiceAux, infos)) {
				...
				params.put(
						"PaymentRule_1",
						infoOnCredit != null ? order.getPaymentTerm().getName() : MRefList.getListName(getCtx(), MPOSPaymentMedium.TENDERTYPE_AD_Reference_ID,
								MPOSPaymentMedium.TENDERTYPE_CreditNote));
				params.put("PaymentRule_Amt_1", infoOnCredit != null ? infoOnCredit.getAmount() : currentAccountInfo.getAmount());
				ProcessInfo info = MProcess.execute(getCtx(), MInvoice.getCurrentAccountDocumentProcessID(null), params, null);
				if (info.isError()) {
	...
	public List<PaymentMedium> getPaymentMediums() {
		...
		List<MPOSPaymentMedium> mPaymentMediums = MPOSPaymentMedium.getAvailablePaymentMediums(getCtx(), null, MPOSPaymentMedium.CONTEXT_CustomerReceiptsOnly,
				true, getTrxName());
		PaymentMedium paymentMedium = null;
		int discountSchemaID = 0;
		for (MPOSPaymentMedium mposPaymentMedium : mPaymentMediums) {
			// Crea la instancia del medio de pago con los datos del
			// C_POSPaymentMedium
			paymentMedium = new PaymentMedium(mposPaymentMedium.getID(), mposPaymentMedium.getName(), mposPaymentMedium.getTenderType(),
					mposPaymentMedium.getC_Currency_ID());
			// Si tiene esquema de descuento se obtiene la instancia y se asocia
			// al
			// medio de pago.
			...
			// Si es un medio de pago de tipo Tarjeta de Crédito se guarda el ID
			// de
			// la entidad financiera y se cargan los planes de tarjeta
			// disponibles
			// para la misma
			if (paymentMedium.isCreditCard()) {
				paymentMedium.setEntidadFinanciera(getEntidadFinanciera(mposPaymentMedium.getM_EntidadFinanciera_ID()));
				paymentMedium.setCreditCardPlans(getCreditCardPlans(paymentMedium.getEntidadFinanciera().getId()));
			}

			// Si es un medio de pago de tipo Cheque se guarda el plazo de cobro
			if (paymentMedium.isCheck()) {
				paymentMedium.setCheckDeadLine(Integer.parseInt(mposPaymentMedium.getCheckDeadLine()));
				paymentMedium.setValidationBeforeCheckDeadLines(mposPaymentMedium.isValidateBeforeCheckDeadLines());
				paymentMedium.setBeforeCheckDeadLineFrom(mposPaymentMedium.getBeforeCheckDeadLineFrom() != null ? Integer.parseInt(mposPaymentMedium
						.getBeforeCheckDeadLineFrom()) : null);
				paymentMedium.setBeforeCheckDeadLineTo(mposPaymentMedium.getBeforeCheckDeadLineTo() != null ? Integer.parseInt(mposPaymentMedium
						.getBeforeCheckDeadLineTo()) : null);
				paymentMedium.setBeforeCheckDeadLinesToValidate(getBeforeCheckDeadLinesToValidate(mposPaymentMedium));
			}
			...
			// Se asigna el nombre del tipo de pago a partir de la lista de
			// referencia
			paymentMedium.setTenderTypeName(MRefList.getListName(ctx, MPOSPaymentMedium.TENDERTYPE_AD_Reference_ID, paymentMedium.getTenderType()));
	...
	/**
	 * Devuelve una lista con todos los planes válidos para la fecha actual de
	...
	protected List<EntidadFinancieraPlan> getCreditCardPlans(int entidadFinancieraID) {
		...
		List<MEntidadFinancieraPlan> mCreditCardPlans = MEntidadFinancieraPlan.getPlansAvailables(getCtx(), entidadFinancieraID, getTrxName());
		...
		for (MEntidadFinancieraPlan mEntidadFinancieraPlan : mCreditCardPlans) {
			...
			plan = new EntidadFinancieraPlan(mEntidadFinancieraPlan.getID(), entidadFinancieraID, mEntidadFinancieraPlan.getName(),
	...
	/**
	 * Busca la lista de Cajas Diarias abiertas para un usuario en una
	 * organización determinada, indicando además la fecha de validez de la
	 * misma.
	 * 
	 * @param orgID
	 *            ID de organización
	...
	private List<PoSConfig> getJournalPOSConfigs(int orgID, int userID, Timestamp date) {
		...
		String sql = "SELECT * " + "FROM C_POSJournal " + "WHERE AD_Org_ID = ? " + "AND AD_User_ID = ? " + "AND DateTrx::date = ?::date "
				+ "AND DocStatus = ? ";
	...
	public List<PaymentTerm> getPaymentTerms() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				pt = new PaymentTerm(rs.getInt("c_paymentterm_id"), rs.getString("name"), rs.getInt("c_pospaymentmedium_id"));
				pts.add(pt);
	...
	public PaymentMedium getPaymentMedium(Integer paymentMediumID) {
		...
		if (!Util.isEmpty(paymentMediumID, true)) {
			MPOSPaymentMedium posPM = new MPOSPaymentMedium(getCtx(), paymentMediumID, getTrxName());
			pm = new PaymentMedium(posPM.getID(), posPM.getName(), posPM.getTenderType(), posPM.getC_Currency_ID());
		}
		return pm;
	}

	private void debug(String text) {
		System.out.println("TPV DEBUG ==> " + DB.getSQLValueTimestamp(null, "select now()") + " - " + text);
	}
	...
	protected String createTrxName() {
		return Trx.createTrxName(this.toString() + invoiceDate.toString() + Thread.currentThread().getId());
	}
	...
	public void voidDocuments() throws PosException {
		...
		try {
			...
			// MAllocationHdr vAlloc = new MAllocationHdr(getCtx(),
			// allocHdr.getC_AllocationHdr_ID(), trxName);
			// vAlloc.setAllocationAction(MAllocationHdr.ALLOCATIONACTION_VoidPayments);
			// if (!DocumentEngine.processAndSave(vAlloc,
			// MAllocationHdr.ACTION_Void, false)) {
			...
			// MInOut vShipment = new MInOut(getCtx(), shipment.getM_InOut_ID(),
			// trxName);
			// if (!DocumentEngine.processAndSave(vShipment, MInOut.ACTION_Void,
			// false)) {
			...
			// MInvoice vInvoice = new MInvoice(getCtx(),
			// invoice.getC_Invoice_ID(), trxName);
			// if (!DocumentEngine.processAndSave(vInvoice,
			// MInvoice.ACTION_Void, false)) {
			...
			// MOrder vOrder = new MOrder(getCtx(), morder.getC_Order_ID(),
			// trxName);
			// if (!DocumentEngine.processAndSave(vOrder, MOrder.ACTION_Void,
			// false)) {
			// throw new PosException(vOrder.getProcessMsg());
			// }
			// }

			InvoiceGlobalVoiding voidingProcess = new InvoiceGlobalVoiding(invoice.getC_Invoice_ID(), getCtx(), trxName);
	...
	public Integer getMaxOrderLineQty() {
		...
		if (LOCAL_AR_ACTIVE) {
			...
			String maxQtyValue = MPreference.GetCustomPreferenceValue(MAX_ORDER_LINE_QTY_PREFERENCE_NAME, Env.getAD_Client_ID(getCtx()));
			maxQty = Util.isEmpty(maxQtyValue, true) ? null : Integer.valueOf(maxQtyValue);
		}
		return maxQty;
	}
	...
	public MDocType getLocaleArDocType() throws PosException {
		...
		// Se obtiene el tipo de documento para la factura.
		return MDocType.getDocType(ctx, Env.getAD_Org_ID(getCtx()), MDocType.DOCTYPE_CustomerInvoice, mLetraComprobante.getLetra(), posNumber, getTrxName());
	}
	...
	public MLetraComprobante getLocaleArLetraComprobante() throws PosException {
		...
		Integer categoriaIVACustomer = partner == null ? 0 : partner.getC_Categoria_Iva_ID();
		...
		// Se obtiene el ID de la letra del comprobante a partir de las
		// categorias de IVA.
	...
	private void loadCheckDeadLines() {
		List<String> values = MRefList.getValueList(getCtx(), MPOSPaymentMedium.CHECKDEADLINE_AD_Reference_ID, getTrxName());
	...
	@Override
	public boolean isCheckDeadLineInRange(Integer checkDeadLineToCompare, Integer checkDeadLineFrom, Integer checkDeadLineTo, Integer checkDeadLineActual) {
		return checkDeadLineFrom <= checkDeadLineToCompare
				&& (checkDeadLineTo != null ? (checkDeadLineTo >= checkDeadLineToCompare) : (checkDeadLineActual > checkDeadLineToCompare));
	}
	...
	protected List<Integer> getBeforeCheckDeadLinesToValidate(MPOSPaymentMedium posPaymentMedium) {
		...
		if (posPaymentMedium.isValidateBeforeCheckDeadLines()) {
			...
			for (Integer deadline : getCheckDeadLines()) {
				if (isCheckDeadLineInRange(
						deadline,
						Integer.parseInt(posPaymentMedium.getBeforeCheckDeadLineFrom()),
						!Util.isEmpty(posPaymentMedium.getBeforeCheckDeadLineTo(), true) ? Integer.parseInt(posPaymentMedium.getBeforeCheckDeadLineTo()) : null,
	...
	public boolean hasCreditNotesAvailables(Integer bpartnerID, boolean excludeCreditNotes) {
		...
		return MInvoice.hasCreditsOpen(getCtx(), bpartnerID, true, excludedCredits, getTrxName());
	}
	...
	public void setCreatePOSPaymentValidations(CreatePOSPaymentValidations createPOSPaymentValidations) {
	...
	protected void setCompleteOrderPOSValidations(CompleteOrderPOSValidations completeOrderPOSValidations) {
