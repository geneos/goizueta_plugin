...
public class BPartnerDialog extends JDialog {
	...
	private CPanel getCDataPanel() {
		if (cDataPanel == null) {
			...
			cDataPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(
	...
	private VLookup getCBPartner() {
		if (cBPartner == null) {
			...
			cBPartner.setValue(getPoS().getModel().getOrder().getBusinessPartner() == null ? getPoS().getModel().getDefaultBPartner() : getPoS().getModel()
	...
	/**
	 * Verifico que todos los productos de la orden de compra esten dentro de la
	 * tarifa seleccionada
	 */
	 ...
	 * @return lista de precios del combo con ese id o null en caso que no se
	 *         encuentre
	 */
	...
	private boolean manageBPartner() {
		boolean ok = false;
		if ((getCBPartner().getValue() != null) && getCBPartner().isEnabled()) {
			// Load BPartner
