...
import ar.com.geneos.goizueta.plugin.client.pos.model.IPaymentMediumInfo;
...
public class PoSMainForm extends CPanel implements FormPanel, ASyncProcess, Disposable {
	...
	public void initialAuthorization() throws PosException {
		if (getModel().getPoSConfig().isInitialAuthorization()) {
			AuthOperation authOperation = new AuthOperation(UserAuthConstants.POS_INIT_UID, getMsgRepository().getMsg(
					UserAuthConstants.getProcessValue(UserAuthConstants.POS_INIT_UID)), UserAuthConstants.POS_INIT_MOMENT);
	...
	protected void initMsgs() {
		...
		// Estos mensajes no se asignan a variables de instancias dado que son
		// mensajes
		// devueltos por el modelo del TPV, pero se realiza la invocación a
		// getMsg(...) para
		// obtenerlos desde la BD y que queden cacheados en el Repositorio de
		// mensajes.
		// De esta forma se preserva la lógica para Modo Offline de TPV, la cual
		// tiene como
	...
	private void initialize() {
		...
		setManualDiscountAuthOperation(new AuthOperation(UserAuthConstants.POS_MANUAL_GENERAL_DISCOUNT_UID, MSG_DISCOUNT_GENERAL,
				UserAuthConstants.POS_FINISH_MOMENT));
		getManualDiscountAuthOperation().setLazyAuthorization(true);
		getFrame().setTitle(getFrame().getTitle() + "- " + getModel().getPoSConfig().getName());
	}

	private void keyBindingsInit() {
		// Deshabilito el F10 que algunos look and feel y
		// técnicas de focos asignan al primer componente menú de la barra de
		// menú
		getFrame().getJMenuBar().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_F10, 0), "none");
		...
		getActionMap().put(UPDATE_ORDER_PRODUCT_ACTION, new AbstractAction() {
		...
		getActionMap().put(PAY_ORDER_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
		...
		getActionMap().put(GOTO_PAYMENTS_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
		...
		getActionMap().put(ADD_PAYMENT_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
		...
		// Accion: Abrir el diálogo para ingresar/editar los datos del
		// comprador.
		getActionMap().put(SET_CUSTOMER_DATA_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (!isProcessing()) {
		...
		// Accion: Abrir el diálogo para editar los datos del cliente, la
		// tarifa, etc.
		getActionMap().put(SET_BPARTNER_INFO_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				openBPInfoDialog();
			}
		});
		...
		getActionMap().put(GOTO_ORDER, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (!isProcessing()) {
		...
		getActionMap().put(ADD_ORDER_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				addCustomerOrder();
			}
		});
		...
		getActionMap().put(REMOVE_PAYMENT_ACTION, new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (!isProcessing()) {
					removePayment();
				}
	 ...
	 */
	protected CPanel getCOrderPanel() {
		if (cOrderPanel == null) {
	...
	private CPanel getCProductInPanel() {
		if (cProductInPanel == null) {
			...
			cProductInPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory
					.createLineBorder(java.awt.Color.gray, 1), MSG_PRODUCT_INCOME, javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
	...
	private CPanel getCTotalPanel() {
		if (cTotalPanel == null) {
			...
			cTotalPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createEmptyBorder(0, 3, 0, 0),
					javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(java.awt.Color.gray, 1), MSG_TOTAL,
							javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog",
									java.awt.Font.PLAIN, 12), new java.awt.Color(51, 51, 51))));
	...
	private VLookup getCProductLookup() {
		if (cProductLookup == null) {
			...
			// Solo se muestra el botón del lookup si el perfil logueado tiene
			// permisos para
	...
	public JTable getCOrderTable() {
		if (cOrderTable == null) {
			...
			// Renderer de SenderDetails (Agregado Goizueta).
			cOrderTable.getColumnModel().getColumn(7).setCellRenderer(new DefaultTableCellRenderer() {
	...
	private CPanel getCClientPanel() {
		if (cClientPanel == null) {
			...
			cGeneralDiscountLabel
					.setText("%" + " " + MSG_DISCOUNT_GENERAL_SHORT + " " + KeyUtils.getKeyStr(getActionKeys().get(CHANGE_FOCUS_GENERAL_DISCOUNT)));
			...
			// cClientPanel.add(getCCustomerDescriptionText(),
		...
		if (cGeneralDiscountPercText == null) {
			...
			cGeneralDiscountPercText.addVetoableChangeListener(new VetoableChangeListener() {
				...
				public void vetoableChange(PropertyChangeEvent event) throws PropertyVetoException {
					...
					// Agregar o Actualizar el descuento manual general por el
					// valor del porcentaje del monto
					BigDecimal percentage = (BigDecimal) event.getNewValue() == null ? BigDecimal.ZERO : (BigDecimal) event.getNewValue();
			...
			// cGeneralDiscountPercText.addAction("updateAmount",
	 ...
	 * @return org.compiere.swing.CPanel
	 */
	protected CPanel getCPaymentCenterPanel() {
		if (cPaymentCenterPanel == null) {
			cPaymentCenterPanel = new CPanel();
			// cPaymentCenterPanel.setLayout(new
			// BoxLayout(getCPaymentCenterPanel(), BoxLayout.X_AXIS));
	...
	private CPanel getCClientBorderPanel() {
		if (cClientBorderPanel == null) {
			...
			cClientBorderPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createTitledBorder(
					javax.swing.BorderFactory.createLineBorder(java.awt.Color.gray, 1), MSG_CLIENT, javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
					javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", java.awt.Font.PLAIN, 12), new java.awt.Color(51, 51, 51)),
	...
	private CPanel getCTenderTypes() {
		if (cTenderTypes == null) {
			...
			cTenderTypes.setBorder(javax.swing.BorderFactory.createCompoundBorder(
	...
	private CPanel getCPaymentMediumInfoPanel() {
		if (cPaymentMediumInfoPanel == null) {
			...
			cPaymentMediumInfoPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(0, 6, 0, 0),
					BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), BorderFactory.createEmptyBorder(0, 5, 5, 5))));
	...
	private CPanel getCCreditCardInfoPanel() {
		if (cCreditCardInfoPanel == null) {
			...
			// cCreditCardInfoPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(0,5,0,0),
	...
	private CPanel getCSelectTenderTypeContentPanel() {
		if (cSelectTenderTypeContentPanel == null) {
			...
			// cSelectTenderTypeContentPanel.setPreferredSize(new
			// java.awt.Dimension(300,200));
	...
	private CPanel getCCreditCardParamsPanel() {
		if (cCreditCardParamsPanel == null) {
			...
			// cCreditCardParamsPanel.setPreferredSize(new
			// java.awt.Dimension(S_TENDERTYPE_PANEL_WIDTH,155)); //107
		...
		// contenido en un panel a la vez, con lo cual cada vez que se muestra
		// el panel
	...
	private CPanel getCCreditNoteParamsPanel() {
		if (cCreditNoteParamsPanel == null) {
			...
			// GridBagConstraints userAuthConstraints = new
			// GridBagConstraints();
			...
			// cCreditNoteParamsPanel.setPreferredSize(new
			// java.awt.Dimension(S_TENDERTYPE_PANEL_WIDTH,155)); // 47
			...
			// cCreditNoteParamsPanel.add(getCCashRetunAuthPanel().getAuthPanel(),
			// userAuthConstraints);
		}
	...
	private VCheckBox getCCreditNoteCashReturnCheck() {
		if (cCreditNoteCashReturnCheck == null) {
			...
			cCreditNoteCashReturnCheck.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent arg0) {
					VCheckBox checkToCompare = arg0 == null ? cCreditNoteCashReturnCheck : ((VCheckBox) arg0.getSource());
					...
					// manageMaxCashReturnValue((BigDecimal)
					// getCCreditNoteCashReturnAmtText()
					// .getValue());
				}
			});
			cCreditNoteCashReturnCheck.addAction("setSelected", KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), new AbstractAction() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
					cCreditNoteCashReturnCheck.setSelected(!((VCheckBox) arg0.getSource()).isSelected());
	...
	private VNumber getCCreditNoteCashReturnAmtText() {
		if (cCreditNoteCashReturnAmtText == null) {
			...
			cCreditNoteCashReturnAmtText.addAction("updateAmount", KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), new AbstractAction() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
					if (getCCreditNoteCashReturnAmtText().getValue() == null) {
						getCCreditNoteCashReturnAmtText().setValue(getCCreditNoteBalanceText().getValue());
	...
	private CPanel getCTransferParamsPanel() {
		if (cTransferParamsPanel == null) {
			...
			// cTransferParamsPanel.setPreferredSize(new
			// java.awt.Dimension(S_TENDERTYPE_PANEL_WIDTH,155)); //74
		...
		// El combo de cuenta bancaria siempre se agrega fuera del If ya que el
		// combo
		// es compartido con el panel de Cheques. Un componente solo puede estar
		// contenido en un panel a la vez, con lo cual cada vez que se muestra
		// el panel
	...
	private VNumber getCAmountText() {
		if (cAmountText == null) {
			...
			cAmountText.addAction("updateAmount", KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), new AbstractAction() {
				@Override
				public void actionPerformed(ActionEvent arg0) {
					if (getCAmountText().getValue() == null) {
						getCAmountText().setValue(getOrder().getOpenAmount());
		...
		if (cCardText == null) {
			...
			cCardText.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					List<EntidadFinanciera> financieras = getModel().getEntidadesFinancieras(creditCardStr);
					loadTenderTypePaymentMediums(MPOSPaymentMedium.TENDERTYPE_CreditCard, financieras);
					if (financieras.isEmpty()) {
						selectedOld = selectedOld == null ? getCPaymentMediumCombo().getItemAt(0) : selectedOld;
	...
	private CPanel getCCheckParamsPanel() {
		if (cCheckParamsPanel == null) {
			...
			// cCheckParamsPanel.setPreferredSize(new
			// java.awt.Dimension(S_TENDERTYPE_PANEL_WIDTH,155));
		...
		// El combo de cuenta bancaria siempre se agrega fuera del If ya que el
		// combo
		// es compartido con el panel de Transferencias. Un componente solo
		// puede estar
		// contenido en un panel a la vez, con lo cual cada vez que se muestra
		// el panel
		// de Cheque o Transferencia hay que agregar el combo al panel mostrado
	...
	private CPanel getCCreditParamsPanel() {
		if (cCreditParamsPanel == null) {
			...
			// cCashParamsPanel.setPreferredSize(new
			// java.awt.Dimension(S_TENDERTYPE_PANEL_WIDTH,155)); //20
	...
	private CPanel getCPaymentTotalBorderPanel() {
		if (cPaymentTotalBorderPanel == null) {
			...
			cPaymentTotalBorderPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createTitledBorder(
					javax.swing.BorderFactory.createLineBorder(java.awt.Color.gray, 1), MSG_PAYMENTS_SUMMARY,
					javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog",
							java.awt.Font.PLAIN, 12), new java.awt.Color(51, 51, 51)), javax.swing.BorderFactory.createEmptyBorder(0, 10, 1, 10)));
	...
	private CPanel getCPaymentTotalPanel() {
		if (cPaymentTotalPanel == null) {
			...
			// cPaymentTotalPanel.setPreferredSize(new
			// java.awt.Dimension(340,75));
	...
	private CButton getCFinishPayButton() {
		if (cFinishPayButton == null) {
			...
			// cFinishPayButton.setPreferredSize(new
			// java.awt.Dimension(100,26));
	...
	private CScrollPane getCTenderTypesTableScrollPane() {
		if (cTenderTypesTableScrollPane == null) {
			...
			// cTenderTypesTableScrollPane.setPreferredSize(new
			// java.awt.Dimension(190,200));
	...
	private CPanel getCLoadOrderPanel() {
		if (cLoadOrderPanel == null) {
			...
			cLoadOrderPanel.setBorder(javax.swing.BorderFactory.createCompoundBorder(javax.swing.BorderFactory.createTitledBorder(
					javax.swing.BorderFactory.createLineBorder(java.awt.Color.gray, 1), MSG_LOAD_CUSTOMER_ORDER, javax.swing.border.TitledBorder.LEFT,
					javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", java.awt.Font.PLAIN, 12), new java.awt.Color(51, 51, 51)),
	...
	private CPanel getCCashReturnPanel() {
		if (cCashReturnPanel == null) {
			...
			cCashReturnPanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(0, 6, 0, 0),
					BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED), BorderFactory.createEmptyBorder(0, 5, 5, 5))));
	...
	public boolean addOrderProduct(Product product) {
		...
		try {
			...
			// Si la excepción tiene título y descripción se muestran estos
			// datos
	...
	private void searchProduct(String code) {
		...
		// 1. No se ingresó un texto de búsqueda, se abre el buscador de
		// artículos.
		if (noCode) {
			openInfo = true;

			// 2. No se encontraron artículos en la búsqueda, se informa al
			// usuario mediante un
			// mensaje
			...
			// 3. Se encontró un único artículo (ya sea por match exacto o
			// parcial), se
			...
			if (productList.getExactProductCount() == 1) {
				...
				// Si hay mas de un resultado exacto pero el usuario no tiene
				// acceso al buscador
				// de artículos, entonces se obtiene el primer artículo
				// resultante por matching
				...
				// Aquí se sabe que hay mas de 1 resultado, 0 o mas de un
				// exacto. En este
		...
		if (product != null) {
			...
			// Si se determinó abrir el buscador (porque hay mas de un artículo
			// encontrado) solo
	...
	/**
	 * @param orderTableUtils
	 *            Fija o asigna orderTableUtils.
	 */
	...
	/**
	 * @param tenderTypeTableUtils
	 *            Fija o asigna tenderTypeTableUtils.
	 */
	...
	private void openCustomerDataDialog() {
		...
		// TODO: FB. Falta definir cuando es obligatorio la identificación del
		// comprador
		// y cuando no para ver cual es el valor del parámetro mandatoryData en
		// la instanciación
		// del diálogo.
	...
	protected void alertAutomaticCreditNote() {
		if (getCPosTab().getSelectedIndex() == 1 && getOrder().getBusinessPartner().isAutomaticCreditNote() && getModel().hasCreditNotesAvailables(false)) {
	...
	protected void cancelOrder() {
		if (hasOrderProducts() && askMsg(MSG_CONFIRM_CANCEL_ORDER)) {
			// Si el TPV está configurado para permitir modificaciones de
			// precios
			// entonces no se validan los datos de usuario.
			if (!getModel().priceModifyAllowed()) {
				// Validar permisos de usuario
				AuthOperation authOperation = new AuthOperation(UserAuthConstants.POS_CANCEL_ORDER_UID, MSG_CANCEL_ORDER,
	 ...
	 *            Fija o asigna componentFactory.
	 */
	...
	protected boolean loadBPartner(int bPartnerID) {
		...
		if (getOrder().getBusinessPartner() == null || getOrder().getBusinessPartner().getId() != bPartnerID) {
		...
		// setCustomerDataVisible(getOrder().getBusinessPartner().getIVACategory()
	...
	private void refreshBPartnerDiscount() {
		...
		getCBPartnerDiscountText().setText(getDiscountSchemaDescription(discountSchema));
	}
	...
	private void updateCheckTenderTypeComponents() {
		...
		refreshPaymentMediumInfo();
		getCBankAccountCombo().setValue(getModel().getPoSConfig().getCheckBankAccountID());
	}
	...
	private void updateCreditCardTenderTypeComponents() {
		...
		getCCashReturnPanel().setVisible(
				getModel().getPoSConfig().isAllowCreditCardCashRetirement()
						&& (getSelectedPaymentMedium() != null && getSelectedPaymentMedium().getEntidadFinanciera() != null && getSelectedPaymentMedium()
								.getEntidadFinanciera().isCashRetirement()));
	...
	private void addPayment() {
		...
		if (paymentMedium == null) {
			...
			return;
			// Si el balance es positivo implica que no hay mas nada que pagar
			// (ya se
			// han agregado medios de pago que cubren el importe total del
			// pedido)
		...
		} else if (!MPOSPaymentMedium.TENDERTYPE_CreditNote.equals(tenderType) && getOrder().getBusinessPartner().isAutomaticCreditNote()
		...
		if (MPOSPaymentMedium.TENDERTYPE_Cash.equals(tenderType)) {
			...
			else if (paymentMedium.isValidationBeforeCheckDeadLines() && !getModel().existsBeforeCheckDeadLinesFor(paymentMedium)) {
			...
			payment = new CheckPayment(bankName, checkNumber, emissionDate, acctDate, bankAccountID, paymentMedium.getCheckDeadLine());
			payment.setTenderType(tenderType);
			payment.setTypeName(paymentMedium.getName());
			((CheckPayment) payment).setCuitLibrador(cuitLibrador); // Si locale
																	// AR no
																	// está
																	// activo
																	// esto es
			...
			// (EntidadFinanciera)getCCreditCardCombo().getValue();
			// <-
			...
			cashRetirementAmt = paymentMedium.getEntidadFinanciera().isCashRetirement() ? cashRetirementAmt : BigDecimal.ZERO;
			...
			if (getSelectedCreditCardPlan() == null) {
				...
				return;
				// El importe de retiro de efectivo es mayor al límite
				// configurado
			} else if (cashRetirementAmt.compareTo(paymentMedium.getEntidadFinanciera().getCashRetirementLimit()) > 0) {
				errorMsg(MSG_CASH_RETIREMENT_EXCEEDS_LIMIT);
				return;
			}
			extraValidationsResult = getExtraPOSPaymentAddValidations().validateCreditCardPayment(this, creditCardNumber, couponNumber, couponBatchNumber,
					cashRetirementAmt);
			if (extraValidationsResult.isError()) {
				errorMsg(extraValidationsResult.getMsg());
				return;
			...
			payment = new CreditCardPayment(creditCardPlan, creditCardNumber, couponNumber, bankName, posnet, couponBatchNumber, cashRetirementAmt);
			...
			if (invoiceID == null || invoiceID == 0) {
				...
				return;
				// El importe no puede ser mayor al monto pendiente de la nota
				// de crédito
			...
			} else if (returnCash && (returnCashAmt == null || returnCashAmt.compareTo(BigDecimal.ZERO) <= 0)) {
				errorMsg(MSG_RETURN_CASH_POSITIVE_AMOUNT_ERROR);
				return;
				// La devolución en efectivo debe ser menor o igual al saldo que
				// queda de la NC
			} else if (returnCash && returnCashAmt != null && returnCashAmt.compareTo(balanceAmt) > 0) {
				errorMsg(MSG_RETURN_CASH_AMOUNT_ERROR);
				return;
				// Si la devolución en efectivo se debe controlar y supera el
				// máximo permitido, entonces se debe autorizar
			} else if (getModel().isControlCashReturns() && returnCash && (returnCashAmt != null && getModel().isCashReturnedSurpassMax(returnCashAmt))) {
				AuthOperation authOperation = new AuthOperation(UserAuthConstants.POS_CN_MAX_CASH_RETURN_UID, getMsgRepository().getMsg(
						UserAuthConstants.getProcessValue(UserAuthConstants.POS_CN_MAX_CASH_RETURN_UID)), UserAuthConstants.POS_ADD_PAYMENT_MOMENT);
				getAuthDialog().addAuthOperation(authOperation);
			...
			extraValidationsResult = getExtraPOSPaymentAddValidations().validateCreditNotePayment(this, balanceAmt, returnCash, returnCashAmt);
			if (extraValidationsResult.isError()) {
				errorMsg(extraValidationsResult.getMsg());
				return;
			}
	...
	private void completeOrder() {
		...
		if (getOrder().getBusinessPartner().isAutomaticCreditNote() && getModel().hasCreditNotesAvailables(true)
		...
		// final Waiting waitingDialog = new Waiting(getFrame(),waitMsg +
		// "...",false,60);
		...
		SwingWorker worker = new SwingWorker() {
			...
			public Object construct() {
				...
				try {
					...
					errorDesc = Util.isEmpty(e.getMessage()) ? e.getCause().getMessage() : e.getMessage();
	...
	private void selectTab(int index) {
		...
		if (index == 0) {
			...
			// Se carga el cliente que tenga asignado el pedido. (pedidos pre
			// creados)
			if (getOrder().getBusinessPartner() != null) {
			...
			// Actualiza la info adicional del medio de pago porque puede haber
			// cambiado
	...
	/**
	 * Carga el pedido de cliente y muestra los datos del encabezado en los
	 * controles de la interfaz. No carga las líneas del pedido de cliente al
	 * pedido de TPV, esto se realiza luego mediante el método
	 * {@link #addCustomerOrder()}
	 * 
	 * @param orderID
	 *            ID del pedido de cliente
	 */
	...
	protected DocActionStatusListener docActionStatusListener = new DocActionStatusListener() {
		...
		public void docActionStatusChanged(DocActionStatusEvent event) {
			...
			if (event.getDocActionStatus() == DocActionStatusEvent.ST_FISCAL_PRINT_DOCUMENT) {
				...
				// Se setea la ventana de informaciÃ³n tanto como listener de
				// estado de la
	...
	/**
	 * Agrega el pedido de cliente actualmente cargado al pedido del TPV (agrega
	 * los artículos del pedido de cliente al pedido del TPV).
	 */
	private void addCustomerOrder() {
		...
		try {
			...
			// Si el pedido que se agrega tiene una EC diferente a la actual
			// entonces
			// se guarda el ID de esa EC para luego asignarla como EC actual.
			if (getModel().getCustomerOrder().getBusinessPartner().getId() != getOrder().getBusinessPartner().getId()) {
				newBPartnerID = getModel().getCustomerOrder().getBusinessPartner().getId();
			}
	...
	/**
	 * Reliza el requerimiento de foco de un component bajo el hilo de Swing.
	 * 
	 * @param component
	 *            Componente al cual se le quiere dar el foco.
	 */
	 ...
	 * 
	 * @param selectTenderType
	 *            Tipo de pago a seleccionar
	 */
	private void selectTenderType(String selectTenderType) {
		...
		// Carga el tipo de pago seleccionado (tal como se hace cuando el
		// usuario
	...
	private void loadTenderType(String tenderType) {
		// Obtiene el panel que muestra los parámetros personalizados del tipo
		// de pago
		// y quita todos los controles actuales para insertar los del tipo
		// seleccionado.
		...
		// Realiza la carga del medio de pago seleccionado por defecto en el
		// combo (siempre
	...
	private void loadTenderTypePaymentMediums(String tenderType, List<EntidadFinanciera> entidadesFinancieras) {
		if (entidadesFinancieras.isEmpty()) {
			...
			for (PaymentMedium paymentMedium : paymentMediums) {
				...
				if (paymentMedium.getTenderType().equals(tenderType) && paymentMedium.getEntidadFinanciera() != null
	...
	/**
	 * Actualiza los componentes que contienen los parámetros específicos para
	 * un tipo de pago determinado.
	 * 
	 * @param tenderType
	 *            Tipo de pago seleccionado.
	 */
	...
	/**
	 * Carga los parámetros del medio de pago seleccionado. Si no hay ningún
	 * medio de pago seleccionado en el combo el método no realiza ningún
	 * cambio.
	 */
	private void loadPaymentMedium(boolean updateComponents) {
		...
		if ((paymentMedium.isCheck() || paymentMedium.isCreditCard()) && paymentMedium.hasBank()) {
	...
	/**
	 * Borra el contenido de los controles que muestran la información adicional
	...
	private void loadPaymentMediumInfo() {
		...
		// Si el MP es tarjeta de crédito, entonces el esquema de descuento se
		// obtiene
		...
		// Muestra el esquema de descuento y monto final del pago (cero por
		// defecto)
	...
	private void refreshPaymentMediumInfo(BigDecimal amt) {
		...
		// componente.
		BigDecimal paymentToPayAmt = getOrder().getToPayAmount(getSelectedPaymentMediumInfo(), amt);

		getCPaymentToPayAmt().setValue(paymentToPayAmt.compareTo(BigDecimal.ZERO) > 0 ? paymentToPayAmt : null);

		// BigDecimal paymentRealAmt =
		// getOrder().getPaymentRealAmount(paymentToPayAmt,
		...
		if (paymentMedium.isCreditCard()) {
			...
			BigDecimal cuotaAmt = getToPaymentAmount().divide(new BigDecimal(plan.getCoutasPago()), 10, BigDecimal.ROUND_HALF_UP);
	...
	/**
	 * @return El plan de tarjeta seleccionado en el ComboBox (solo para medios
	 *         de pago de tipo tarjeta)
	 */
	...
	private boolean validateCheckAcctDate(Date acctDate, PaymentMedium paymentMedium) {
		...
		// La fecha de vto no puede ser mayor a la fecha actual sumada la
		// cantidad de días
		...
		if (acctDate.compareTo(maxAcctDate) > 0) {
			errorMsg(MSG_INVALID_CHECK_ACCTDATE + ":", DisplayType.getDateFormat(DisplayType.Date).format(maxAcctDate));
			return false;
		}
	...
	private void loadCreditNote(int invoiceID) {
		...
		// Carga el campo de importe con el importe pendiente en caso de que el
		// usuario
		// no haya ingresado un valor aún
		BigDecimal amount = (BigDecimal) getCAmountText().getValue();
		if (amount == null || amount.compareTo(BigDecimal.ZERO) == 0 || amount.compareTo(availableAmt) > 0) {
	...
	/**
	 * Valida si el monto de un pago no supera el monto a pagar del pedido. En
	 * caso de superarlo asigna el monto a pagar o el paymentLimitAmount en el
	 * campo de importe según corresponda
	 * 
	 * @param amount
	 *            Importe del pago en cuestión
	 * @param paymentLimitAmount
	 *            Importe límite del pago. Este es el importe máximo que puede
	 *            usarse para el pago. Por ejemplo en Notas de Crédito, este
	 *            valor sería el monto pendiente de asignación. Si el pago no
	 *            tiene límite este parámetro debe ser <code>null</code>. Si el
	 *            importe del pago supera el monto a pagar y el pago tiene un
	 *            límite, se asigna este límite al campo de importe. Si no tiene
	 *            límite se asigna entonces el monto a pagar del pedido.
	 * @return Devuelve <code>true</code> si el importe es válido,
	 *         <code>false</code> en caso contrario.
	 */
	private boolean validatePaymentAmount(BigDecimal amount, BigDecimal paymentLimitAmount) {
		boolean valid = true;
		// El importe no puede superar el pendiente a pagar según el medio de
		// pago.
	...
	protected void updateCreditNoteBalance() {
		...
		BigDecimal balanceCreditNote = (availableCreditNoteAmt == null || availableCreditNoteAmt.compareTo(BigDecimal.ZERO) == 0) || amt == null ? null
				: availableCreditNoteAmt.subtract(amt);
		// Agregar el saldo del crédito que quedará al final de la operación
		getCCreditNoteBalanceText().setValue(
				balanceCreditNote == null ? null : (balanceCreditNote.compareTo(BigDecimal.ZERO) >= 0 ? balanceCreditNote : BigDecimal.ZERO));
		getCCreditNoteCashReturnCheck().setVisible(balanceCreditNote != null && balanceCreditNote.compareTo(BigDecimal.ZERO) > 0);
	 ...
	 * 
	 * @param paramsPanel
	 *            Panel de parámetros a mostrar
	 * @param infoPanel
	 *            Panel de información adicional. Puede ser <code>null</code>
	 */
	...
	/**
	 * Devuelve la descripción a mostrar para un esquema de descuento. Si el
	 * esquema es <code>null</code> devuelve un String especial.
	 * 
	...
	/**
	 * @return El importe del pago ingresado por el usuario. Si el usuario no
	 *         ingresó ningún importe devuelve cero.
	 */
	...
	protected void createInfoFiscalPrinter() {
		infoFiscalPrinter = new AInfoFiscalPrinter(null, getWindowNo(), Msg.parseTranslation(Env.getCtx(), "@PrintingFiscalDocument@"));

		infoFiscalPrinter.setDialogActionListener(new AInfoFiscalPrinter.DialogActionListener() {
			...
			public Object construct() {
				try {
					...
					if (!getModel().getPoSConfig().isVoidDocuments()) {
						AuthOperation authOperation = new AuthOperation(UserAuthConstants.POS_VOID_DOCUMENTS_UID, getMsgRepository().getMsg(
								UserAuthConstants.getProcessValue(UserAuthConstants.POS_VOID_DOCUMENTS_UID)), UserAuthConstants.POS_VOID_DOCUMENT);
						getAuthDialog().addAuthOperation(authOperation);
			...
			public void finished() {
				...
				if (!success) {
					...
					if (askMsg(MSG_RETRY_VOID_INVOICE, MSG_RETRY_VOID_INVOICE_INFO
							+ (getModel().getPoSConfig().isPosJournal() ? " " + MSG_RETRY_VOID_INVOICE_INFO_POS_JOURNAL : ""))) {
	...
	public void updateAllowClose() {
		updateCloseApp(getModel().getConfig().isLockedClosed() && getOrder().getOrderProducts().size() > 0 ? "N" : "Y");
	}

	public void updateCloseApp(String value) {
		if (getModel().getConfig() != null && getModel().getConfig().isLockedClosed()) {
			...
			// (value.equals("N")?"none":"End"));
			// getFrame().getJMenuBar().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			// KeyStroke.getKeyStroke(KeyEvent.VK_X,Event.SHIFT_MASK +
			// Event.ALT_MASK), (value.equals("N")?"none":"Exit"));
		}
	}
	...
	public void setExtraPOSPaymentAddValidations(AddPOSPaymentValidations extraPOSPaymentAddValidations) {
	...
	protected void setManualDiscountAuthOperation(AuthOperation manualDiscountAuthOperation) {
	...
	protected void refreshBPartnerCreditPaymentMedium(String tenderType) {
		if (MPOSPaymentMedium.TENDERTYPE_Credit.equals(tenderType)) {
			getCPaymentMediumCombo().removeAllItems();
			getCPaymentMediumCombo().addItem(getOrder().getBusinessPartner().getPaymentMedium());
		}
	}
