...
public class CurrentAccountQuery {
	...
	public String getQuery() {
		...
		// Geneos 20/09/2016 
		// Solución al bug que se da cuando la fecha de contabilización tiene hora diferente a 00:00:00 y que no toma los registros.
		
		//sql.append(" AND fa.DateAcct >= ").append(DB.TO_DATE(p_DateAcct_From)).append(" AND DateAcct < ").append(DB.TO_DATE(p_DateAcct_To)).append(" + INTERVAL '1 day'");
				
		
		//sqlAppend("   AND d.Dateacct::date < (? ::date + INTERVAL '1 day') ", getDateTo(), sql);
		sql.append("  AND d.Dateacct::date < ").append(DB.TO_DATE(getDateTo())).append(" + INTERVAL '1 day'");
