...
public class WSFEConsultarComprobanteProcess extends SvrProcess {
	...
	protected String archivo_salida_url = "/home/geneos/salida.txt";
	protected String archivo_errores_url = "/home/geneos/errores.txt";
		...
		*/
		...
		if(response.getResultGet().getObservaciones() != null) {
			res_strr = "Comprobante: " + response.getResultGet().getCbteDesde() + " - " +
					...
					"CAE: " + response.getResultGet().getCodAutorizacion() + " - " +
		...
		try {
			archivo_salida.write(retValues.toString() + lineSeparator);
		} catch (IOException e) {
		...
		if(cbte.length != 0){
			...
			if(String.valueOf(inv.getPuntoDeVenta()).compareTo(ptoVtaStr) != 0 || String.valueOf(inv.getNumeroComprobante()).compareTo(nroCbteStr) != 0){
				...
				*/
				...
				String txt_err = "Error en datos ptoVta: " + ptoVta + " nroCbte: " + nroCbte + " CAE: " + CAE + lineSeparator;
				
				try {
					archivo_errores.write(txt_err);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			try {
				String txt_err = "Error inexistente ptoVta: " + ptoVta + " nroCbte: " + nroCbte + " CAE: " + CAE + lineSeparator;
				archivo_errores.write(txt_err);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
