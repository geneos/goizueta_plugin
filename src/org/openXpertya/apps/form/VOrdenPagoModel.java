...
public class VOrdenPagoModel {
	...
	private void agregarRetencionesComoMediosPagos(Vector<MedioPago> pagos,
		...
		String sql = " SELECT C_Invoice_ID FROM m_retencion_invoice WHERE C_AllocationHdr_ID = " + hdr.getC_AllocationHdr_ID();
		...
		/*
		 * 	Validación para no agregar las retenciones en cero.
		 * 	12/06/2017
		 *  Geneos 
		 * 
		 */
		...
		while (rs.next()) {
			...
			for (MedioPago mp : pagos) {
				if (mp.getTipoMP().equals(MedioPago.TIPOMEDIOPAGO_CREDITO)) {
					MedioPagoCredito mpc = (MedioPagoCredito) mp;
					if (mpc.C_invoice_ID == invoice.getC_Invoice_ID() && !mpc.getImporte().equals(Env.ZERO)) {
						// pagos.remove(mpc);
						mpe.add(mpc);
					}
				}
			...
			if(!rc.getImporte().equals(Env.ZERO))
				pagos.add(rc);
		}
