...
//		String sql = MRole.getDefault().addAccessSQL(" SELECT C_Tax_ID FROM C_Tax WHERE isactive = 'Y' AND istaxexempt = 'Y' AND to_country_id IS NULL AND rate = 0.0 ", "C_Tax", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);;
	...
	*/
	...
	public boolean save(MAllocationHdr alloc) throws Exception {
		...
		/*
		 * Geneos
		 * Comentado para que guarde un registro aún sin que emita retención para casos de bug detectado con
		 * medios de pago que no sean cajas.
		 * 
		...
		*/
		...
		/*
		 * Geneos
		 * Agreagado para que no genere los comprobantes cuando guarde un registro aún sin que emita retención para casos de bug detectado con
		 * medios de pago que no sean cajas.
		 *  
		 */
